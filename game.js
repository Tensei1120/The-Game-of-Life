(function () {
  ...
  const SQ_ACROSS = 160;  // 横方向（垂直コラムの左右）の半幅 ×2がマス高さ
  let SQ_ALONG = 40;
  let ROAD_W = 182;
  const MAX_PLAYERS = 9;
  ...
  // 盤面: 4列縦型スネーク　左上=START, 右上=GOAL
  // 列x: 100, 400, 700, 1000  　y範囲: 90〜840
  // SQ_ACROSS/2=80px が境界との余白 => 左端100-80=20, 右端1000+80=1080, 上端90-80=10, 下端840+80=920 すべてキャンバス内
  const WAYPOINTS = [
    // 列1 (x=100): 上→下  (START 左上)
    [100,  90],
    [100, 460],
    [100, 840],
    // 下コネクタ
  ...
  const branchSquares = (function() {
    ...
    const SAMPLES=600;
    const arcSamples=[{t:0,len:0}]; let arcTotal=0;
  ...
  function drawBranchLabels(){
    const j0=branchSquares.job[4], u0=branchSquares.uni[4];
    if(!j0||!u0)return;
    ctx.save();
    ctx.font='bold 12px Segoe UI';ctx.textAlign='center';ctx.textBaseline='middle';
    // 就職ルートラベル
    const jx=j0.cx, jy=j0.cy-SQ_ACROSS*0.6;
    ...
    // 大学ルートラベル
    const ux=u0.cx, uy=u0.cy-SQ_ACROSS*0.6;
  ...
  function drawSquare({num,cx,cy,corners}){
    ...
    // グロス
    ctx.save();ctx.globalAlpha=0.32;ctx.fillStyle='rgba(255,255,255,0.9)';
    ctx.beginPath();
    ctx.moveTo(corners[0].x,corners[0].y);
    ctx.lineTo(corners[1].x,corners[1].y);
    ...
    if(isGoal){
      ctx.fillStyle='#8a6000';ctx.font='bold 14px Segoe UI';ctx.fillText('GOAL',cx,cy-9);
      ctx.font='20px serif';ctx.fillText('🏆',cx,cy+10);
    } else if(isStart){
      ...
      ctx.font='bold 11px Segoe UI';ctx.fillText(num,cx,cy+8);
    } else {
  ...
  function drawBranchSquare({num,cx,cy,corners,route}){
    ...
    quadPath(corners);ctx.fill();ctx.restore();
    ctx.strokeStyle=isJob?'#c06810':'#2840b8';ctx.lineWidth=1.5;
    quadPath(corners);ctx.stroke();
  ...
  function tokenOffset(total,i){
    ...
    return{x:Math.cos(a)*14,y:Math.sin(a)*14};
  }
